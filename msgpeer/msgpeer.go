...
import (
	...
	"github.com/someonegg/bmsgpump"
)

type Request = bmsgpump.Message
type Response = bmsgpump.Message
type Notify = bmsgpump.Message
...
//
// The write methods of bmsgpump.Pump like Output and Post should not be called.
func NewPeer(rw bmsgpump.MessageReadWriter, h Handler, writeQueueSize int) *Peer {
	p := &Peer{
		h:     h,
		resps: make(map[string]chan Response),
	}
	p.Pump = bmsgpump.NewPump(rw, p, writeQueueSize)
	return p
}
...
func (p *Peer) Do(ctx context.Context, r Request) (Response, error) {
	...
	err := p.Pump.OutputMP(ctx, bmsgpump.MPMessage{requestHeader(rid), r})
	if err != nil {
		return nil, err
	}
	...
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-p.Pump.StopD():
		return nil, bmsgpump.ErrPumpStopped
	case resp := <-respC:
		added = false
		return resp, nil
	}
...
func (p *Peer) Notify(ctx context.Context, n Notify) error {
	h := []byte("N\n")
	return p.Pump.OutputMP(ctx, bmsgpump.MPMessage{h, n})
}

// Process implements the bmsgpump.Handler interface.
//
...
//	P,request-id\n    for response
//	N\n               for notify
func (p *Peer) Process(ctx context.Context, m bmsgpump.Message) {
	const MaxHeader = 128
	...
	case "R":
		...
		p.h.Process(ctx, r,
			func(ctx context.Context, resp Response) error {
				return p.Pump.OutputMP(ctx, bmsgpump.MPMessage{responseHeader(rid), resp})
			})
	case "P":
		rid := ss[1]
		p.locker.Lock()
